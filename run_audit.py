...
#
# FOUNDRY:
#   Offline mode is ALWAYS supplied with --offline.
#   "offline = true" is automatically removed from foundry.toml.
# ============================================================
...
# Do NOT use FOUNDRY_OFFLINE=1.
# Foundry offline mode is supplied explicitly via --offline.
os.environ.pop("FOUNDRY_OFFLINE", None)
...
# ============================================================
# AUTOMATIC FOUNDRY CONFIGURATION REPAIR
# ============================================================

def repair_foundry_config():

    if not FOUNDRY_CONFIG.exists():

        print(
            "⚠️ foundry.toml does not exist.",
            flush=True
        )
    ...
    try:
        ...
        original = FOUNDRY_CONFIG.read_text(
            encoding="utf-8",
            errors="replace"
        )
        ...
        for line in lines:
            ...
            # Remove invalid offline configuration.
            if stripped in (
                "offline = true",
                "offline = false"
            ):
        ...
        if removed:
            ...
            print(
                "   Removed offline = true/false.",
                flush=True
            )

            print(
                "   Offline mode uses --offline CLI flag.",
                flush=True
            )
            ...
            print(
                "✅ foundry.toml offline setting is clean.",
                flush=True
            )
        ...
        print(
            f"❌ Unable to repair foundry.toml: {exc}",
            flush=True
        )
...
def run_opencode_live(prompt):
    ...
    print("RPC: DISABLED", flush=True)
    print("Deployment: DISABLED", flush=True)
    print("Broadcast: DISABLED", flush=True)
    print("Gas: ZERO", flush=True)
    print("Git push: DISABLED", flush=True)
...
IMPORTANT FOUNDRY CONFIGURATION:

NEVER add this to foundry.toml:
...
Offline mode MUST be supplied through the CLI:
...
If foundry.toml contains an offline setting, remove it.
...
banner("🌳 ARIZONA FORESTATION DAO")
...
print(f"\nRepository : {REPO}", flush=True)
print(f"Model      : {MODEL}", flush=True)
print("RPC        : DISABLED", flush=True)
print("Deployment : DISABLED", flush=True)
print("Broadcast  : DISABLED", flush=True)
print("Transactions: DISABLED", flush=True)
print("Gas        : ZERO", flush=True)
print("Git push   : DISABLED", flush=True)
print("Mode       : LOCAL / OFFLINE", flush=True)
...
print(f"Forge   : {FORGE}", flush=True)
...
# ============================================================
# FIX FOUNDRY CONFIG BEFORE ANY FORGE COMMAND
# ============================================================

banner("🔧 FOUNDRY CONFIGURATION SAFETY CHECK")

repair_foundry_config()
...
for attempt in range(
    ...
    # Repair Foundry configuration before every attempt.
    repair_foundry_config()
...
# ============================================================
# FINAL FOUNDRY CONFIGURATION REPAIR
# ============================================================

banner("🔧 FINAL FOUNDRY CONFIGURATION CHECK")
...
print("Broadcast    : DISABLED", flush=True)
print("Transactions : DISABLED", flush=True)
print("Live chain   : NOT TOUCHED BY THIS WRAPPER", flush=True)
print("Gas          : ZERO", flush=True)
print("Git push     : DISABLED", flush=True)
